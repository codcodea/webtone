import { createSignal, Setter, Switch, Match, createEffect } from "solid-js"
...
const PatternPortal = (props: PatternPortalProps) => {
    ...
    const setPattern = () => {
        const scale = (Math.random() * 4 + 3).toFixed(2)
        ...
        <Portal>
            ...
            >
                <div class="canvas h-full w-full p-12">
                    <Switch>
                        <Match when={patternNo() == 0}>{<Zebra ref={setPatternEl} />}</Match>
                        <Match when={patternNo() == 1}>{<Herringbone ref={setPatternEl} />}</Match>
                        <Match when={patternNo() == 2}>{<Blobs ref={setPatternEl} />}</Match>
                        <Match when={patternNo() == 3}>{<Squares ref={setPatternEl} />}</Match>
                        <Match when={patternNo() == 4}>{<Leafs ref={setPatternEl} />}</Match>
                        <Match when={patternNo() == 5}>{<Flower ref={setPatternEl} />}</Match>
                        <Match when={patternNo() == 6}>{<S6ties ref={setPatternEl} />}</Match>
                        <Match when={patternNo() == 7}>{<ManyLeafs ref={setPatternEl} />}</Match>
                        <Match when={patternNo() == 8}>{<Tribal ref={setPatternEl} />}</Match>
                    </Switch>
                </div>
...
function validSVGIndex(clones: number): number[] {
    const components = []
    if (clones === 2) {
        components.push(0)
    }
...
const Herringbone = (props: SvgProps) => {
    return (
...
const ManyLeafs = (props: SvgProps) => {
    return (
        <svg id="patternId" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
            <defs>
                <pattern
