...
var _ColorSpace = class _ColorSpace {
  constructor(options) {
    ...
    this.coords = coords;
    let white6 = options.white ?? this.base.white ?? "D65";
...
var π2 = Math.PI;
var d2r2 = π2 / 180;
function deltaECMC_default(color, sample, { l = 2, c: c4 = 1 } = {}) {
  [color, sample] = getColor([color, sample]);
  let [L1, a1, b1] = lab_default.from(color);
...
function display(color, { space = defaults_default.display_space, ...options } = {}) {
  let ret = serialize(color, options);
  if (typeof CSS === "undefined" || CSS.supports("color", ret) || !defaults_default.display_space) {
    ret = new String(ret);
    ret.color = color;
  } else {
...
function contrastLstar(color1, color2) {
  color1 = getColor(color1);
  color2 = getColor(color2);
  let L1 = get(color1, [lab_default, "l"]);
  let L2 = get(color2, [lab_default, "l"]);
...
function contrastDeltaPhi(color1, color2) {
  color1 = getColor(color1);
  color2 = getColor(color2);
  let Lstr1 = get(color1, [lab_d65_default, "l"]);
  let Lstr2 = get(color2, [lab_d65_default, "l"]);
...
function contrast(background, foreground, o = {}) {
  ...
  if (!algorithm) {
    let algorithms = Object.keys(contrast_exports).map((a2) => a2.replace(/^contrast/, "")).join(", ");
    throw new TypeError(`contrast() function needs a contrast algorithm. Please specify one of: ${algorithms}`);
  }
  background = getColor(background);
  foreground = getColor(foreground);
  for (let a2 in contrast_exports) {
...
function uv(color) {
  let [X, Y, Z] = getAll(color, xyz_d65_default);
...
function deltaE(c14, c24, o = {}) {
  if (isString(o)) {
    o = { method: o };
  }
  let { method = defaults_default.deltaE, ...rest } = o;
  for (let m3 in deltaE_default) {
...
// node_modules/colorjs.io/src/variations.js
...
function mix(c14, c24, p2 = 0.5, o = {}) {
  [c14, c24] = [getColor(c14), getColor(c24)];
  if (type(p2) === "object") {
    [p2, o] = [0.5, p2];
  }
  let r = range(c14, c24, o);
  return r(p2);
}
function steps(c14, c24, options = {}) {
  let colorRange;
  if (isRange(c14)) {
    [colorRange, options] = [c14, c24];
    [c14, c24] = colorRange.rangeArgs.colors;
  }
  let {
    maxDeltaE,
    deltaEMethod,
  ...
  if (!colorRange) {
    [c14, c24] = [getColor(c14), getColor(c24)];
    colorRange = range(c14, c24, rangeOptions);
  }
  let totalDelta = deltaE(c14, c24);
...
var hpluv_default = new ColorSpace({
  ...
  toBase(hsl) {
    ...
    if (l > 99.9999999) {
      ...
      let lines = calculateBoundingLines(l);
      let max2 = calcMaxChromaHpluv(lines, h);
      c4 = max2 / 100 * s;
    }
    return [l, c4, h];
  },
...
var rec2100_pq_default = new RGBColorSpace({
  ...
  toBase(RGB) {
    return RGB.map(function(val) {
      let x = (Math.max(val ** minv - c13, 0) / (c23 - c33 * val ** minv)) ** ninv2;
      return x * 1e4 / Yw2;
    });
  },
  fromBase(RGB) {
    return RGB.map(function(val) {
      let x = Math.max(val * Yw2 / 1e4, 0);
  ...
  fromBase(RGB) {
    return RGB.map(function(val) {
      if (val <= 0) {
        ...
        return (Math.log2(ε10 + val * 0.5) + 9.72) / 17.52;
      } else {
...
export {
  a98rgb_default as A98RGB,
  a98rgb_linear_default as A98RGB_Linear,
  acescc_default as ACEScc,
  acescg_default as ACEScg,
  cam16_default as CAM16_JMh,
  ColorSpace,
  hct_default as HCT,
  hpluv_default as HPLuv,
  hsl_default as HSL,
  hsluv_default as HSLuv,
  hsv_default as HSV,
  hwb_default as HWB,
  Hooks,
  ictcp_default as ICTCP,
  jzczhz_default as JzCzHz,
  jzazbz_default as Jzazbz,
  lch_default as LCH,
  lchuv_default as LCHuv,
  lab_default as Lab,
  lab_d65_default as Lab_D65,
  luv_default as Luv,
  oklch_default as OKLCH,
  oklab_default as OKLab,
  p3_default as P3,
  p3_linear_default as P3_Linear,
  prophoto_default as ProPhoto,
  prophoto_linear_default as ProPhoto_Linear,
  rec2020_default as REC_2020,
  rec2020_linear_default as REC_2020_Linear,
  rec2100_hlg_default as REC_2100_HLG,
  rec2100_pq_default as REC_2100_PQ,
  RGBColorSpace,
  xyz_abs_d65_default as XYZ_ABS_D65,
  xyz_d50_default as XYZ_D50,
  xyz_d65_default as XYZ_D65,
  clone,
  contrast,
  contrastAPCA,
  contrastDeltaPhi,
  ...
  deltaE,
  deltaE2000_default as deltaE2000,
  deltaE76,
  deltaECMC_default as deltaECMC,
  deltaEHCT_default as deltaEHCT,
  deltaEITP_default as deltaEITP,
  deltaEJz_default as deltaEJz,
  deltaE_default as deltaEMethods,
  deltaEOK_default as deltaEOK,
  display,
  distance,
  equals,
  get,
  ...
  parse,
  range,
  srgb_default as sRGB,
  srgb_linear_default as sRGB_Linear,
  serialize,
  set,
  setAll,
  setLuminance,
...
//# sourceMappingURL=colorjs__io_fn.js.map
